            ...
            (
                ...
                OrderedDict(
                    [
                        (
                            "nconst",
                            Column(name="person_id", type="VARCHAR PRIMARY KEY"),
                        ),
            ...
            (
                ...
                OrderedDict(
                    [
                        ...
                        (
                            "isOriginalTitle",
                            Column(name="is_original_title", type="INTEGER"),
                        ),
                    ]
                ),
            ),
            ...
            (
                ...
                OrderedDict(
                    [
                        (
                            "tconst",
                            Column(name="episode_title_id", type="INTEGER", index=True),
                        ),
                        (
                            "parentTconst",
                            Column(name="show_title_id", type="INTEGER", index=True),
                        ),
                        ("seasonNumber", Column(name="season_number", type="INTEGER")),
                        (
                            "episodeNumber",
                            Column(name="eposide_number", type="INTEGER"),
                        ),
                    ]
                ),
            ),
...
class Database:
    ...
    def _create_index_sql(table_name, columns):
        lines = [
            "CREATE INDEX ix_{table}_{col} ON {table} ({col});".format(
                table=table_name, col=c.name
            )
            for c in columns
...
def main():
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show database interaction"
    )
    ...
    if os.path.exists(opts.db):
        logger.warning(
            "DB already exists: ({db}). Refusing to modify. Exiting".format(db=opts.db)
        )
        return 1
